using namespace std;

#include "stdio.h"
#include "TFile.h"
#include <fstream>
#include <iostream>
#include <TChain.h>
#include "TLeaf.h"
#include "TH1.h"
#include "TH2.h"
#include "TTree.h"
#include "TMath.h"
#include "TProfile.h"
#include "TProfile2D.h"
#include "TObjArray.h"
#include "TNtuple.h"
#include "TRandom.h"
#include "TRandom3.h"
#include "TVector2.h"

const float PI = TMath::Pi();

// EPOS particle ids (see analyze.C / idt.dt)
const int ID_Kp   = 130;    // K+
const int ID_Km   = -130;   // K-
const int ID_p    = 1120;   // proton
const int ID_pbar = -1120;  // antiproton

// track cuts
const float etaCut = 1.0;  // |eta| < etaCut for v2(pT)
const float ptMin  = 0.05; // minimum pT

void Elliptic_KP(int cen = 5, int job = 0){	//main_function

    float Eweight = 1;
    TChain* chain = new TChain("teposevent");

    chain->Add("z-*.root");
    //chain->Add("../auau200-6/z-*.root");

    //only read the branches we use -- skips decompressing x,y,z,t,zus,... (about half the file volume)
    chain->SetBranchStatus("*",0);
    const char* activeBranches[] = {"np","bim","npartproj","nparttarg","phi","phir","px","py","pz","id","ist"};
    for(int ib = 0; ib < 11; ib++) chain->SetBranchStatus(activeBranches[ib],1);

    char fname_out[200];
    sprintf(fname_out,"cen%d.v2pT_kaon_proton_job%d.root",cen,job);
    TFile fout(fname_out,"RECREATE");

    //defining histograms
    // v2(pT) = <cos(2(phi - Psi))> vs pT, w.r.t. reaction plane (phir) and participant plane (phir+psi2)
    TProfile* pV2Pt_Kp = new TProfile("pV2Pt_Kp","v_{2}(p_{T}) K^{+} (RP)",60,0,3);
    TProfile* pV2Pt_Km = new TProfile("pV2Pt_Km","v_{2}(p_{T}) K^{-} (RP)",60,0,3);
    TProfile* pV2Pt_K  = new TProfile("pV2Pt_K" ,"v_{2}(p_{T}) K^{#pm} (RP)",60,0,3);
    TProfile* pV2Pt_K_PP = new TProfile("pV2Pt_K_PP","v_{2}(p_{T}) K^{#pm} (PP)",60,0,3);

    TProfile* pV2Pt_p    = new TProfile("pV2Pt_p","v_{2}(p_{T}) p (RP)",60,0,3);
    TProfile* pV2Pt_pbar = new TProfile("pV2Pt_pbar","v_{2}(p_{T}) #bar{p} (RP)",60,0,3);
    TProfile* pV2Pt_pr   = new TProfile("pV2Pt_pr","v_{2}(p_{T}) p+#bar{p} (RP)",60,0,3);
    TProfile* pV2Pt_pr_PP = new TProfile("pV2Pt_pr_PP","v_{2}(p_{T}) p+#bar{p} (PP)",60,0,3);

    // v2 vs eta (integrated over pT)
    TProfile* pV2Eta_K  = new TProfile("pV2Eta_K","v_{2}(#eta) K^{#pm} (RP)",26,-6,6);
    TProfile* pV2Eta_pr = new TProfile("pV2Eta_pr","v_{2}(#eta) p+#bar{p} (RP)",26,-6,6);

    // QA histograms
    TH1D* Hist_Pt_K  = new TH1D("Hist_Pt_K","p_{T} K^{#pm}",60,0,3);
    TH1D* Hist_Pt_pr  = new TH1D("Hist_Pt_pr","p_{T} p+#bar{p}",60,0,3);
    TH1D* Hist_dPhi_K = new TH1D("Hist_dPhi_K","#phi - #Psi_{RP} K^{#pm}",72,-PI,PI);
    TH1D* Hist_dPhi_pr = new TH1D("Hist_dPhi_pr","#phi - #Psi_{RP} p+#bar{p}",72,-PI,PI);
    TH1D* Hist_b = new TH1D("Hist_b","Hist_b",50,0,20);
    TH1D* Hist_Psi = new TH1D("Hist_Psi","#Psi_{RP} (phi)",72,-PI,PI);

    Int_t nentries = chain->GetEntries();
    cout << nentries << "\n";
    for(int i = 0; i < nentries; i++){

        if((i+1)%1000==0) cout << "Processing entry == "<< i+1 <<" == out of "<<nentries<<".\n";
        chain->GetEntry(i);

        TLeaf* leaf_b   = chain->GetLeaf("bim");
        float bim = leaf_b->GetValue(0);
        if(bim < 8.4 || bim > 9.2){	//30-40% centrality
            continue;
        }
        TLeaf* leaf_NoTracks = chain->GetLeaf("np");
        TLeaf* leaf_Np_p= chain->GetLeaf("npartproj");
        TLeaf* leaf_Np_t= chain->GetLeaf("nparttarg");
        int Np = leaf_Np_p->GetValue(0) + leaf_Np_t->GetValue(0);
        int NPTracks = (int)leaf_NoTracks->GetValue(0);

        // event plane: "phi" = random rotation of the event in the lab (reaction plane),
        // "phir" = fluctuation of the participant plane relative to the reaction plane
        TLeaf* leaf_phi  = chain->GetLeaf("phi");
        TLeaf* leaf_phir = chain->GetLeaf("phir");
        float PsiRP = leaf_phi->GetValue(0);
        float PsiPP = PsiRP + leaf_phir->GetValue(0);

        Hist_b->Fill(bim);
        Hist_Psi->Fill(TVector2::Phi_mpi_pi(PsiRP));

        TLeaf* leaf_PxV0       = chain->GetLeaf("px");
        TLeaf* leaf_PyV0       = chain->GetLeaf("py");
        TLeaf* leaf_PzV0       = chain->GetLeaf("pz");
        TLeaf* leaf_PIDV0    = chain->GetLeaf("id");
        TLeaf* leaf_ist        = chain->GetLeaf("ist");

        for(int trk = 0; trk < NPTracks; trk++) {

            int istAsso = (int)leaf_ist->GetValue(trk);
            if (istAsso != 0) continue;	//final state particles only

            int pid = (int)leaf_PIDV0->GetValue(trk);
            bool isKaon   = (pid == ID_Kp || pid == ID_Km);
            bool isProton = (pid == ID_p  || pid == ID_pbar);
            if(!isKaon && !isProton){
                continue;
            }

            float PxAsso    = leaf_PxV0->GetValue(trk);
            float PyAsso    = leaf_PyV0->GetValue(trk);
            float PzAsso    = leaf_PzV0->GetValue(trk);

            float PtAsso = sqrt(PxAsso*PxAsso+PyAsso*PyAsso);
            float PhiAsso = atan2(PyAsso,PxAsso);
            float ThetaAsso = atan2(PtAsso,PzAsso);
            float EtaAsso = -log(tan(ThetaAsso/2.));

            if(PtAsso < ptMin) continue;

            float cos2dPhi_RP = cos(2.*(PhiAsso - PsiRP));
            float cos2dPhi_PP = cos(2.*(PhiAsso - PsiPP));

            if(isKaon){
                pV2Eta_K->Fill(EtaAsso, cos2dPhi_RP, Eweight);
            }
            if(isProton){
                pV2Eta_pr->Fill(EtaAsso, cos2dPhi_RP, Eweight);
            }

            if(EtaAsso > etaCut || EtaAsso < -etaCut) continue;

            if(pid == ID_Kp){
                pV2Pt_Kp->Fill(PtAsso, cos2dPhi_RP, Eweight);
            }
            if(pid == ID_Km){
                pV2Pt_Km->Fill(PtAsso, cos2dPhi_RP, Eweight);
            }
            if(isKaon){
                pV2Pt_K->Fill(PtAsso, cos2dPhi_RP, Eweight);
                pV2Pt_K_PP->Fill(PtAsso, cos2dPhi_PP, Eweight);
                Hist_Pt_K->Fill(PtAsso, Eweight);
                Hist_dPhi_K->Fill(TVector2::Phi_mpi_pi(PhiAsso - PsiRP), Eweight);
            }
            if(pid == ID_p){
                pV2Pt_p->Fill(PtAsso, cos2dPhi_RP, Eweight);
            }
            if(pid == ID_pbar){
                pV2Pt_pbar->Fill(PtAsso, cos2dPhi_RP, Eweight);
            }
            if(isProton){
                pV2Pt_pr->Fill(PtAsso, cos2dPhi_RP, Eweight);
                pV2Pt_pr_PP->Fill(PtAsso, cos2dPhi_PP, Eweight);
                Hist_Pt_pr->Fill(PtAsso, Eweight);
                Hist_dPhi_pr->Fill(TVector2::Phi_mpi_pi(PhiAsso - PsiRP), Eweight);
            }
        }
    }
    fout.Write();
    return;
}
