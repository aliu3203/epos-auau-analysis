...
void analyze(int cen = 5, int job = 0, const Char_t* inFileName = "test.list"){	//main_function
    ...
    TH1D *hMult_All = new TH1D("hMult_All", "Multiplicity (All Particles)", 1000, -0.5, 9999.5); // Revert to 999.5 tmrw
    //TH1D *hPt_All = new TH1D("hPt_All", "Pt (All Particles)", 100, -0.5, 99.5);
    //TH1D *hPhi_All = new TH1D("hPt_All", )

    TH1D *hMult_ptCut = new TH1D("hMult_ptCut", "Multiplicity (pT > 0.05)", 1000, -0.5, 6999.5);
    TH1D *hMult_ptEtaCut = new TH1D("hMult_ptEtaCut", "Multiplicity (pT > 0.05 & |eta| < 0.5)", 100, -0.5, 999.5);
