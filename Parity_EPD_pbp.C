...
const int ID_pip = 2101, ID_pim = -1899, ID_Kp = 1106, ID_Km = -1106, ID_pp = 1001, ID_pb = -1001;
...
const float cenDef[9] = {13.94,13.07,12.21,11.29,10.33,9.25,8,6.56,4.62};   // 200 GeV Au+Au
...
float Eweight = 1;
	...
	int nfile = 0;
	TString mInputFileName = inFileName;
	std::string const dirFile = mInputFileName.Data();
	std::ifstream inputStream( dirFile.c_str() );
	std::string file;
	while(getline(inputStream, file)) nfile += chain->Add(file.c_str());
...
//		TLeaf* leaf_psi = chain->GetLeaf("Psi");
		TLeaf* leaf_b   = chain->GetLeaf("Imp");
                TLeaf* leaf_NoTracks = chain->GetLeaf("Mult");
                TLeaf* leaf_Np_p= chain->GetLeaf("Npartp");
                TLeaf* leaf_Np_t= chain->GetLeaf("Npartt");
...
//		if(cen && Centrality != cen) continue;   	
                ...
                TLeaf* leaf_PIDV0    = chain->GetLeaf("PID");
